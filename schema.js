...
function generateDDL(email, password) {
    ...
    id = db.next_id();
    lid = db.next_id();
    var hashlogin = crypto.createHash('sha1').update(password+config.security.salt+'Admin').digest('hex');
    ...
    passwd = crypto.createHash('sha1').update(email + ':' + hashlogin).digest('hex');
